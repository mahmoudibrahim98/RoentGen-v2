...
class ValidationRunner:
    ...
    def _compute_metrics_batched(self, all_synthetic_images, all_real_images, all_labels, global_step):
        ...
        # length. We therefore:
        #   - Keep FULL tensors for global metrics (FID, BioViL, MS-SSIM).
        ...
        # ------------------------------------------------------------------
        num_images = synth_tensor.shape[0]
        ...
        # Views for label-dependent metrics
        synth_labeled = synth_tensor[:min_label_len]
        real_labeled = real_tensor[:min_label_len] if real_tensor is not None else None
...
def main():
    ...
    # Propagate stop_at_step into config for easier access in loops if desired
    ...
    try:
        ...
        sync_dir = Path(config["output_dir"]) / "validation_sync"
        if accelerator.is_main_process:
            ...
            if sync_file.exists():
                try:
                    sync_file.unlink()
                    logger.info("Removed stale validation_sync/current_checkpoint.json at startup")
                except Exception as e:
